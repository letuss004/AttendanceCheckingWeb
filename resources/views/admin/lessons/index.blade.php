...
@section('content')
        ...
        <div class="row justify-content-center mx-5">
            ...
            <table class="table table-striped">
                ...
                @foreach($users as $student)
                    <tr>
                        <td>{{$loop->iteration}}</td>
                        <td id="stid{{$loop->iteration}}">{{$student->id}}</td>
                        <td>{{$student->name}}</td>
                        <td>{{$student->email}}</td>
                        <td>{{$student->department->department}}</td>
                        <td class="text-center">
                            <a id="dt{{$loop->iteration}}" class="text-decoration-none link-danger"
    ...
    <div class="modal fade" id="delete_student" data-bs-backdrop="static" data-bs-keyboard="false"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure?</p>
                    <input class="d-none" id="course_id_st">
                    <input class="d-none" id="student_id_st">
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button id="delete_student_bt" class="btn btn-danger" data-bs-target="#staticBackdrop"
                            data-bs-dismiss="modal">Delete
                    </button>
                </div>
            </div>
    ...
    <script>
        jQuery(document).ready(function () {
            ...
            @foreach($users as $student)
            jQuery('#dt{{$loop->iteration}}').click(function (e) {
                e.preventDefault();
                document.getElementById("course_id_st").value = "{{$course->id}}";
                document.getElementById("student_id_st").value = document.getElementById("stid{{$loop->iteration}}").innerText;
            });
            @endforeach
            ...
            // delete student
            jQuery('#delete_student').click(function (e) {
                e.preventDefault();
                let course_id = "{{$course->id}}";
                let student_id = document.getElementById("student_id_st").value;
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    }
